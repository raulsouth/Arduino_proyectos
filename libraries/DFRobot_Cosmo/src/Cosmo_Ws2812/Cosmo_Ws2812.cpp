...
/*!
  ...
  @param   t  Pixel type -- add together NEO_* constants defined in
              Cosmo_Ws2812.h, for example NEO_GRB+NEO_KHZ800 for
              NeoPixels expecting an 800 KHz (vs 400 KHz) data stream
              with color bytes expressed in green, red, blue order per
              pixel.
  @return  Cosmo_Ws2812 object. Call the begin() function before use.
*/
Cosmo_Ws2812::Cosmo_Ws2812(uint16_t n, int16_t p, neoPixelType t)
...
/*!
  @brief   Deallocate Cosmo_Ws2812 object, set data pin back to INPUT.
*/
Cosmo_Ws2812::~Cosmo_Ws2812() {
  free(pixels);
  if (pin >= 0)
...
/*!
  @brief   Change the length of a previously-declared Cosmo_Ws2812
  ...
  @note    This function is deprecated, here only for old projects that
           ...
           type).
*/
void Cosmo_Ws2812::updateLength(uint16_t n) {
...
/*!
  @brief   Change the pixel format of a previously-declared
           Cosmo_Ws2812 strip object. If format changes from one of
  ...
  @note    This function is deprecated, here only for old projects that
           ...
           (length, pin, type).
*/
void Cosmo_Ws2812::updateType(neoPixelType t) {
...
#if defined(ARDUINO_ARCH_RP2040)
void Cosmo_Ws2812::rp2040Init(uint8_t pin, bool is800KHz)
{
...
// Not a user API
void  Cosmo_Ws2812::rp2040Show(uint8_t pin, uint8_t *pixels, uint32_t numBytes, bool is800KHz)
{
  if (this->init)
  {
...
*/
void Cosmo_Ws2812::setPin(int16_t p) {
  if (begun && (pin >= 0))
...
*/
void Cosmo_Ws2812::setPixelColor(uint16_t n, uint8_t r, uint8_t g,
                                      uint8_t b) {
...
*/
void Cosmo_Ws2812::setPixelColor(uint16_t n, uint32_t c) {
  if (n < numLEDs) {
...
*/
void Cosmo_Ws2812::fill(uint32_t c, uint16_t first, uint16_t count) {
  uint16_t i, end;
...
*/
uint32_t Cosmo_Ws2812::ColorHSV(uint16_t hue, uint8_t sat, uint8_t val) {
...
*/
uint32_t Cosmo_Ws2812::getPixelColor(uint16_t n) const {
  if (n >= numLEDs)
...
*/
void Cosmo_Ws2812::setBrightness(uint8_t b) {
...
*/
uint8_t Cosmo_Ws2812::getBrightness(void) const { return brightness - 1; }

/*!
  @brief   Fill the whole NeoPixel strip with 0 / black / off.
*/
void Cosmo_Ws2812::clear(void) { memset(pixels, 0, numBytes); }

// A 32-bit variant of gamma8() that applies the same function
// to all components of a packed RGB or WRGB value.
uint32_t Cosmo_Ws2812::gamma32(uint32_t x) {
  uint8_t *y = (uint8_t *)&x;
...
*/
void Cosmo_Ws2812::rainbow(uint16_t first_hue, int8_t reps,
...
/*!
  ...
  @note    This function is declared static in the class so it can be called
           ...
           in the code yet). Use Cosmo_Ws2812::str2order().
*/
neoPixelType Cosmo_Ws2812::str2order(const char *v) {
